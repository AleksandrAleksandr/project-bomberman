...
void GUI::draw(sf::RenderWindow * window) const
{
	...
	if (m_endOfGameMenuView)
	{
		...
		window->draw(*m_returnToMenuButton->GetSpritePointer());
		window->draw(*m_returnToMenuButton->GetTextPointer());

		window->draw(*m_playAgainButton->GetSpritePointer());
		window->draw(*m_playAgainButton->GetTextPointer());

		window->draw(*m_exitButton->GetSpritePointer());
		window->draw(*m_exitButton->GetTextPointer());

		window->draw(*m_winnerText);
	}
	if (m_gameGUIView)
	{
		window->draw(*m_playerOneLives);
		window->draw(*m_playerSecondLives);
	}
...
//	m_playerSecondLives->setColor(sf::Color(255, 255, 255));
	...
	m_frameTexture->loadFromFile("frame.png");
	...
	m_returnToMenuButton = new Botton(sf::Vector2f(screenWidth / 2 - 150, screenHeight / 2.3f - 50), sf::Vector2i(300, 75.f), "pressButton.png", "unpressButton.png", "Return to Menu");
	m_playAgainButton = new Botton(sf::Vector2f(screenWidth / 2 - 150, screenHeight / 1.7f - 50), sf::Vector2i(300, 75.f), "pressButton.png", "unpressButton.png", "Play Again");
	m_exitButton = new Botton(sf::Vector2f(screenWidth / 2 - 150, screenHeight / 1.7f + screenHeight / 1.7f - screenHeight / 2.3f - 50), sf::Vector2i(300, 75.f), "pressButton.png", "unpressButton.png", "Exit Game");
