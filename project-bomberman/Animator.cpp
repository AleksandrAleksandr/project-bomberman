...
bool Animator::AddAnimationState(std::string name, sf::Texture& texture, size_t begin, size_t end, bool autoPlay)
{
	m_statesIterator = m_states.find(name);
	if (m_statesIterator == m_states.end())
	{
		m_states.insert(std::pair <std::string,AnimationStateInfo>(name, AnimationStateInfo() ={texture, begin, end}));
...
bool Animator::AddAnimationState(std::string name, sf::Texture& texture, bool autoPlay)
{
	states.insert(std::pair <std::string, sf::Texture>(name, texture));
...
bool Animator::ChangeActiveState(const std::string & name)
{
	...
	statesIterator = states.find(name);
	if (statesIterator == states.end())
		return false;

	m_activeStateName = statesIterator->first;
	//m_activeStateInfo = &m_statesIterator->second;
	currTexture = statesIterator->second;
	...
	m_sprite->setTexture(currTexture);
